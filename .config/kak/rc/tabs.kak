...
define-command -hidden refresh-buflist-scroll %{
  lua %val{bufname} %sh{echo "$kak_quoted_buflist" | xargs -n1 printf '%s\n'} %sh{tput cols} %opt{modeline_tabs_percentage} %opt{tab_separator} %{
    ...
    local left_sep = (left_shorter and "…" or "") .. " "
    local right_sep = " " .. (right_shorter and "…" or "")

    -- join tabs with separator and formatting
    local tabs = table.concat(merge({left, {"{StatusLineValue}" .. basename(currbuf)}, right}), "{Default} " .. separator .. "{Default} ")
    kak.set_option("buffer", "modeline_buflist", left_sep .. "{Default}" .. tabs .. "{Default}" .. right_sep)
  }
}
